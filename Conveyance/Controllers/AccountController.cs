...
namespace Conveyance.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register()
        {
            ...
            {
                items.Add(new SelectListItem { Text = c.CityName, Value = c.CityID.ToString() });
            }

            ViewBag.DropDownCities = items;
            return View();
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model, string DropDownCities)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ApplicationUser u = UserManager.FindByName(model.UserName);
                    UserManager.AddToRole(u.Id, "Client");
            ...
            // If we got this far, something failed, redisplay form
            ViewBag.ErrorMsg = "Registration failed, please try again later";
            ApplicationDbContext db = new ApplicationDbContext();
            var Cities = db.City.ToList();
            List<SelectListItem> items = new List<SelectListItem>();
            foreach (City c in Cities)
        ...
        {
            ApplicationDbContext db = new ApplicationDbContext();
            List<City> cities = db.City.Where(p=>true).ToList();
            return Json(cities);
        }
